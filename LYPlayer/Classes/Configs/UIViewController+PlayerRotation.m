...
+ (void)load {
    ...
    for (NSUInteger index = 0; index < sizeof(selectors) / sizeof(SEL); ++index) {
        ...
        SEL swizzledSelector = NSSelectorFromString([@"ly_" stringByAppendingString:NSStringFromSelector(originalSelector)]);
        ...
        if (class_addMethod(self, originalSelector, method_getImplementation(swizzledMethod), method_getTypeEncoding(swizzledMethod))) {
            ...
            // class_replaceMethod，它有两种不同的行为。
            // 当类中没有想替换的原方法时，该方法会调用class_addMethod来为该类增加一个新方法，
            // 也因为如此，class_replaceMethod在调用时需要传入types参数，而method_exchangeImplementations和method_setImplementation却不需要。
